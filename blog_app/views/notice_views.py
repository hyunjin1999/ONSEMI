...
def notice_search(request):
    ...
    if search_method == 'title':
        ...
        query = Blog.objects.filter(blog_type='NOTICE', name__icontains=search_query).order_by('-datetime')
    elif search_method == 'both':
        query = Blog.objects.filter(blog_type='NOTICE').filter(Q(title__icontains=search_query) | Q(name__icontains=search_query)).order_by('-datetime')
